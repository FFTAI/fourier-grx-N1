...
- 列出当前的运行模式，使用命令 `fourier-grx list`
- 安装程序软件，使用命令 `fourier-grx install`
- 重新配置启动参数，使用命令 `fourier-grx config`

运行模式清单如下：

| 编号 | 运行模式    | 说明                                 | 终端用户使用 | 开发用户使用  |
|----|---------|------------------------------------|--------|---------|
| 1  | 发布模式    | 适用于机器人出厂使用，默认使用掌机进行机器人控制           | ✅      | ❌       |
| 2  | 调试模式    | 适用于开发者调试使用，默认使用手柄进行机器人控制           | ❌      | ✅       |
| 3  | 遥操作调试模式 | 适用于开发者调试使用，默认使用 Vision Pro 进行机器人控制 | ❌      | ⚠️ 尚未发布 |
| A  | 开发者模式   | 适用于开发者使用 SDK 开发库调试使用               | ❌      | ✅       |
| B  | 离线调试模式  | 适用于远程调试使用，仅供内部测试使用                 | ❌      | ❌       |

### 调试模式

调试模式下，机器人主要通过调用内部算法完成运动功能任务。任务的切换使用手柄的 `L1`、`L2`、`R1`、`R2` 组合实现。
...
目前 Fourier N1 主要支持的功能包括：

| 功能名称       | 任务编号 | 是否包含模块 | 说明   | 手柄快捷键   |
|------------|------|--------|------|---------|
| 站立、行走功能    | 100  |        | 行走任务 | L1 + L2 |
| 站立、行走、跑步功能 | 101  |        | 行走任务 | L1 + L2 |

### 开发者模式

开发者模式下，用户可以参考开源开发接口进行二次开发，
具体的开发接口和示例程序可以参考 [Wiki-GRx-Deploy](https://github.com/FFTAI/Wiki-GRx-Deploy)
